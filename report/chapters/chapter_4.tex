...
The best-performing hyperparameter set from Stage 1 was selected and used to train the final MLP model. This training run recorded the total number of node expansions, the path taken (sequence of weight states), and the overall wall-clock time required for convergence to $L_{\text{target}}$.
...
\noindent This experiment investigated the impact of the target loss value, $L_{\text{target}}$, which is intrinsically linked to the $g(n)$ path cost formulation ($g(n) = (L_{\text{initial}} / I_{\text{max}}) \times k$). Three distinct target values were tested: $L_{\text{target}} \in \{0.1, 0.01, 0.001\}$.
...
\noindent This is the most informative plot for evaluating time-series performance and convergence dynamics. The solid line represents the average loss ($\bar{L}$) across all ten runs at every iteration, clearly demonstrating which method achieves faster convergence or reaches a lower loss plateau on average.
...
\begin{itemize}
    ...
    \item A wide shaded band means the method is volatile ($\sigma$ is large) and highly dependent on the random initialization. A wide band often signals that the algorithm frequently "gets stuck" in poor local minima or conversely, occasionally "hits a jackpot" solution.
\end{itemize}
