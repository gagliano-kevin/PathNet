...
\begin{figure}[H]
    ...
    \includegraphics[width=1.0\textwidth]{figures/sine_tests/sine_final_loss_boxplot_1k.png}
     \caption{Final Loss Box Plot for $I_{\text{max}}=\text{1,000}$ Iterations. The plot summarizes the distribution of terminal loss values.}
\end{figure}
\end{center}
...
\noindent \textbf{Dominance in Performance:} At the $1,000$ iteration mark, the A* Search algorithm demonstrates a clear lead in terms of central tendency. The average loss for A* ($\bar{L} = 0.1353$) is significantly lower than that recorded for Gradient Descent ($\bar{L} = 0.2580$). The stark disparity in Median Loss ($0.0904$ for A* vs. $0.3534$ for GD) suggests that, within this specific iteration window, the heuristic approach is more effective at converging rapidly toward low-loss states, whereas GD appears to suffer from slower convergence or a higher susceptibility to sub-optimal local minima.
...
\begin{figure}[H]
    ...
    \includegraphics[width=1.0\textwidth]{figures/iris_tests/iris_mean_loss_comparison_with_std_1k.png}
     \caption{Mean Loss Trajectory with Standard Deviation Shading for $I_{\text{max}}=\text{1,000}$ Iterations (Iris Dataset).}
\end{figure}
\end{center}
