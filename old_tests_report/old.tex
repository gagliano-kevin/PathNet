...
\title{Mathematical Summary of Previous Tested A* Search Update Strategies and Cost Functions}
\date{}
\begin{document}
...
where $\text{MLP}_n$ is the Quantized MLP model associated with node $n$, and $\text{Loss}$ is the chosen loss function.
...
If $\text{update\_strategy} = 0$, or if $\text{scale\_f} = \text{False}$ (for strategies 1 and 2), the standard A* function is used:
$$f(n) = g(n) + h(n)$$

\subsubsection{Scaled $f$ (for Strategies 1, 2 and 3 with $\text{scale\_f}=\text{True}$)}
