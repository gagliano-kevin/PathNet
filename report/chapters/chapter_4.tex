...
\noindent The optimal hyperparameters found during the previous experiments were adopted for the performance comparison on this dataset:
\begin{itemize}
    \item \textbf{Quantization Factor:} $Q=10$
    \item \textbf{Parameter Range:} $[-10, 10]$
    \item \textbf{Kernels Shape and Stride:} $\text{Weight\_kernel}=(2,2), \text{Bias\_kernel}=(2,1) \text{ and Stride}=2$
...
\noindent The second experiment applies the A* training framework to a more complex classification task, assessing its ability to optimize MLP parameters for the Wine Quality dataset, which features a higher dimensionality and significantly more samples than the previous benchmarks.
...
\begin{itemize}
    ...
    \item \textbf{Output ($y$):} Six quality classes (typically ranging from scores 3 to 8), encoded using one-hot encoding.
    \item \textbf{MLP Architecture:} Input (11 neurons) $\rightarrow$ 
      Hidden Layer 1 (32 neurons, ReLU) $\rightarrow$ 
      Hidden Layer 2 (64 neurons, ReLU) $\rightarrow$ 
      Hidden Layer 3 (32 neurons, ReLU) $\rightarrow$ 
      Output (6 neurons).
\end{itemize}
...
\noindent The optimal hyperparameters identified in Stage 1 were maintained to ensure consistency in the comparative analysis:
\begin{itemize}
    \item \textbf{Quantization Factor:} $Q=10$
    \item \textbf{Parameter Range:} $[-10, 10]$
    \item \textbf{Kernels Shape and Stride:} $\text{Weight\_kernel}=(2,2), \text{Bias\_kernel}=(2,1) \text{ and Stride}=2$
...
\noindent The methodology for visualization and statistical analysis remains identical to that employed for the previous datasets. The comparison between A* and Gradient Descent is performed over ten independent runs ($n=10$) for two iteration budgets: $I_{\text{max}}=1,000$ and $I_{\text{max}}=2,000$. The analysis utilizes Mean Loss with Standard Deviation Shading and Final Loss Box Plot visualizations to determine the efficiency of the A* search in high-dimensional weight spaces.
...
\begin{figure}[H]
    ...
    \includegraphics[width=1.0\textwidth]{figures/wine_tests/wine_mean_loss_comparison_with_std_2k.png}
     \caption{Mean Loss Trajectory with Standard Deviation Shading for $I_{\text{max}}=\text{2,000}$ Iterations (Wine Quality).}
\end{figure}
\end{center}
...
\begin{figure}[H]
    ...
    \includegraphics[width=1.0\textwidth]{figures/wine_tests/wine_final_loss_distribution_comparison_2k.png}
     \caption{Final Loss Box Plot for $I_{\text{max}}=\text{2,000}$ Iterations (Wine Quality).}
\end{figure}
\end{center}

\begin{table}[H]
    \centering
    \caption{Comparative Statistics Over Ten Runs for $I_{\text{max}}=2,000$ Iterations (Wine Quality Dataset)}
...
\noindent \textbf{Conclusion for $I_{\text{max}}=2,000$:} Within the Wine Quality optimization framework, Gradient Descent remains a highly efficient standard, yielding lower error rates with minimal temporal cost. However, the A* approach successfully demonstrates that non-gradient optimization is a viable alternative even for higher-dimensional tasks. While GD currently leads in precision, the results from A* provide a promising foundation, suggesting that with further tuning of the heuristic function or more advanced search pruning, this method could become an increasingly competitive tool for navigating complex regression landscapes.