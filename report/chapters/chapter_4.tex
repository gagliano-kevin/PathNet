...
\noindent \textbf{Conclusion for $I_{\text{max}}=2,000$:} At $2,000$ iterations, Gradient Descent proves to be highly efficient, offering a lower average loss and faster execution. However, the A* method remains a compelling novel alternative; it achieves a best-case minimum loss and a level of statistical consistency (standard deviation) that are nearly on par with the classic approach. While A* currently requires more time to process its search space, its ability to reach competitive high-quality solutions suggests significant potential for the heuristic-driven framework.
\newpage
