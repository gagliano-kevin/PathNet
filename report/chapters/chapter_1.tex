...
\begin{itemize}
    ...
    \item \textbf{Consequence:} This avoids the memory overhead of the Closed Set but leads to the redundant expansion of identical states, severely impacting time efficiency, especially in graphs with many cycles or alternative routes.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item If $h(n)$ is only \textbf{admissible} (or in our case, if both admissibility and consistency are violated), the optimal path to a state might be discovered *after* the state has already been placed in the Closed Set. This necessitates the re-opening mechanism.
    \end{itemize}
\end{itemize}
