...
\subsubsection{Consistency (Monotonicity)}
The condition of \textbf{consistency} is a stronger requirement than admissibility. A heuristic $h(n)$ is consistent if, for every node $n$ and every successor node $n'$ connected by an edge with cost $c(n, n')$, the estimated cost from $n$ to the goal is less than or equal to the cost of moving to $n'$ plus the estimated cost from $n'$ to the goal.
...
\begin{itemize}
    \item \textbf{Optimality Guarantee:} If the heuristic $h(n)$ is $\textbf{admissible}$ (and edge costs are non-negative), A* is guaranteed to find the optimal (lowest-cost) path. If the heuristic is $\textbf{consistent}$, A* is also guaranteed to be optimal and more efficient, as it never needs to process a node more than once.
\end{itemize}
