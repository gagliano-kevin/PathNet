...
\noindent The second experiment applies the A* training framework to a classic regression task, assessing its ability to perform MLP parameter optimization on a high-dimensional, real-world dataset.

\subsection{Dataset and Objective}

The experiment utilizes the California Housing dataset, whichs contains $20,640$ samples across $8$ economic and geographic features, used to predict the median house value in California districts.
...
\noindent The optimal hyperparameters found during the previous phase were adopted for the final comparison phase of this dataset:
\begin{itemize}
    \item \textbf{Quantization Factor:} $Q=10$
    \item \textbf{Parameter Range:} $[-10, 10]$
    \item \textbf{Kernels Shape and Stride:} $\text{Weight\_kernel}=(2,2), \text{Bias\_kernel}=(2,1) \text{ and Stride}=2$
