...
class AdaptiveTrainer:
    def __init__(self, model, loss_fn, quantization_factor, parameter_range, debug_mlp=True, 
                 ...
                 min_bias_kernel=[2],
                 min_stride=1,
                 plateau_patience=50,  # Iterazioni senza miglioramento prima di ridurre
                 reduction_factor=2):   # Fattore di riduzione (divisore)
        ...
        # Kernel e stride iniziali (grandi per esplorazione grossolana)
        self.weight_kernel = weight_kernel
        self.bias_kernel = bias_kernel
        self.x_stride = x_stride
        self.y_stride = y_stride
        ...
        # Tracking per plateau detection
    ...
    def _reduce_kernel_and_stride(self):
        """Riduce kernel e stride quando si raggiunge un plateau."""
        # Riduzione weight kernel
        new_wk = [max(self.min_weight_kernel[0], self.weight_kernel[0] // self.reduction_factor),
                  max(self.min_weight_kernel[1], self.weight_kernel[1] // self.reduction_factor)]
        
        # Riduzione bias kernel
        new_bk = [max(self.min_bias_kernel[0], self.bias_kernel[0] // self.reduction_factor)]
        
        # Riduzione stride (proporzionale al kernel)
        new_x_stride = max(self.min_stride, self.x_stride // self.reduction_factor)
        new_y_stride = max(self.min_stride, self.y_stride // self.reduction_factor)
        
        # Check se c'è stato un cambiamento effettivo
        if (new_wk != self.weight_kernel or new_bk != self.bias_kernel or 
            new_x_stride != self.x_stride or new_y_stride != self.y_stride):
            
            print(f"\n{'='*60}")
            print(f"PLATEAU DETECTED - Reducing kernel and stride:")
            print(f"  Weight Kernel: {self.weight_kernel} → {new_wk}")
            print(f"  Bias Kernel: {self.bias_kernel} → {new_bk}")
            print(f"  X Stride: {self.x_stride} → {new_x_stride}")
            print(f"  Y Stride: {self.y_stride} → {new_y_stride}")
            print(f"{'='*60}\n")
            
            self.weight_kernel = new_wk
            self.bias_kernel = new_bk
            self.x_stride = new_x_stride
            self.y_stride = new_y_stride
    ...
    def train(self, X, Y):
        ...
        initial_mlp = QuantizedMLP(self.model, self.loss_fn, self.quantization_factor, 
                                self.parameter_range, debug=self.debug_mlp)
        initial_loss = initial_mlp.evaluate(X, Y)
        ...
        self.g_costs[initial_hash] = initial_node.g_val
        self.best_node = initial_node
        self.last_best_loss = initial_loss
        ...
        for iteration in range(self.max_iterations):
            ...
            # Tracking miglioramenti per plateau detection
            improvement = self.last_best_loss - current_node.h_val
            
            if current_node.h_val < self.best_node.h_val:
                self.best_node = current_node
                print(f"Iteration {iteration+1}: New best loss = {self.best_node.h_val}")
            
            # Increment counter SEMPRE, reset solo se c'è miglioramento significativo
            if improvement > 1e-5:  # Soglia di miglioramento significativo
                self.iterations_without_improvement = 0
                self.last_best_loss = current_node.h_val
            else:
                self.iterations_without_improvement += 1

            # Adaptive kernel reduction su plateau
            if (self.adaptive_kernel and 
                self.iterations_without_improvement >= self.plateau_patience):
                
                if self._reduce_kernel_and_stride():
                    self.iterations_without_improvement = 0  # Reset dopo riduzione
                    # Aggiorna last_best_loss per ripartire dal livello attuale
                    self.last_best_loss = self.best_node.h_val

            if (iteration + 1) % self.log_freq == 0:
                print(f"Iteration {iteration+1}: Best current loss = {self.best_node.h_val} | No improvement for {self.iterations_without_improvement} iters")
                
            neighbors = get_neighbors(current_node, X, Y, self.quantization_factor, 
                                    self.weight_kernel, self.bias_kernel, 
                                    self.x_stride, self.y_stride, self.delta_abs)
